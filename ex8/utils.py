...
def multivariateGaussian(X, mu, Sigma2):
    ...
    ----------
    X : array_like
        The dataset of shape (m, n). Where there are m examples of n-dimensions.
    mu : array_like
    ...
    Sigma2 : array_like
        ...
        (i.e. it is the diagonal of the correlation matrix), or the full correlation
        matrix of shape (n, n) which can represent dependent features.
...
def visualizeFit(X, mu, sigma2):
    ...
    """
    X1, X2 = np.meshgrid(np.arange(0, 30.5, 0.5), np.arange(0, 30.5, 0.5))
...
def checkCostFunction(cofiCostFunc, lambda_=0.):
    ...
    numgrad = computeNumericalGradient(lambda x: cofiCostFunc(x, Y, R,
        num_users, num_movies, num_features, lambda_), params)
...
def loadMovieList():
    ...
    for movie in movies:
        parts = movie.split()
        moviesNames.append(' '.join(parts[1:]).strip())
    return moviesNames
