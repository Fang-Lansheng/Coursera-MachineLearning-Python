...
def plotData(X, y, grid=False):
    """
    Plots the data points X and y into a new figure. Uses `+` for positive examples,
    ...
    y : numpy ndarray
        The data labels.
    grid : bool, optional
        Specify whether or not to show the grid in the plot.
...
def svmTrain(X, y, C, kernelFunction, tol=1e-3, max_passes=5, args=()):
    ...
    ---------
    X : numpy ndarray
        (m x n) Matrix of training examples. Each row is a training example, and
        the jth column holds the jth feature.
    Y : numpy ndarray
        (m, ) A vector (1-D numpy array) containing 1 for positive examples and
        0 for negative examples.
    C : float
        The standard SVM regularization parameter.
    kernelFunction : func
        A function handle which computes the kernel. The function should accept
        two vectors as inputs, and returns a scalar as output.
    tol : float, optional
        Tolerance value used for determining equality of floating point numbers.
    max_passes : int, optional
        Controls the number of iterations over the dataset (without changes to
        alpha) before the algorithm quits.
    args : tuple
        Extra arguments required for the kernel function, such as the sigma
        parameter for a Gaussian kernel.
    ...
    # We have implemented the optimized vectorized version of the Kernels here
    # so that the SVM training will run faster
    if kernelFunction.__name__ == 'linearKernel':
        # Vectorized computation for the linear regression
    ...
    while passes < max_passes:
        ...
        for i in range(m):
            ...
            if (y[i] * E[i] < -tol and alphas[i] < C) or (y[i] * E[i] > tol and alphas[i] > 0):
                ...
                # objective function positive definite, there will be a minimum along the
                # direction of linear equality constrain, and eta will be greater than zero.
                ...
                b2 = b - E[j] - y[i] * (alphas[i] - alpha_i_old) * K[i, j] \
                     - y[j] * (alphas[j] - alpha_j_old) * K[j, j]   # is that right?
...
def svmPredict(model, X):
    ...
    """
    # check if we are getting a vector. If so, then assume we only need to do
...
def visualizeBoundary(X, y, model):
    ...
    ----------
    X : array_like
        (m, 2) The training data with two features (to plot in a 2-D plane).
    y : array_like
        (m, ) The data labels.
    model : dict
        Dictionary of model variables learned by SVM.
